...
| `belongs_to_group` | Student | Group | — | Членство в группе | 🆕 NEW |
...
- **Transitive**: `is_subcompetency_of` (уже есть).
...
*Ограничение шаблона — два операнда.* SWRL Horn-правила проверяют конкретное число атомов. Для 3 операндов — отдельный шаблон (три `has_subpolicy`, два `DifferentFrom`). Для n операндов либо n шаблонов, либо композиция: `AND(a, b, c) := AND(a, AND(b, c))`. Решение для ПЗ: реализуем 2-арный и 3-арный шаблоны напрямую, больше — через композицию (это чистый синтаксический сахар на уровне API, пользователь видит плоский список; PolicyService разворачивает в дерево).
...
| 3 | viewed_required | Шаблон 3 | satisfies | — |
| 4 | competency_required | Шаблон 4 (+ H-1) | satisfies | — |
| 5 | date_restricted | Шаблон 5 | satisfies | CurrentTime, has_value, valid_from, valid_until |
| 6 | and_combination | Шаблон 6 (2/3-арный) | satisfies | has_subpolicy |
| 7 | or_combination | Шаблон 7 | satisfies | has_subpolicy |
| 8 | group_restricted | Шаблон 8 | satisfies | Group, belongs_to_group, restricted_to_group |
...
Общее число SWRL-правил в каталоге: **11** (9 атомарных/композитных + 1 вспомогательное H-1 + 1 мета-правило ступени 2). Плюс расширение шаблона 6 на 3-арность — ещё 1 правило при необходимости.
...
**Группы:** `grp_standard`, `grp_advanced`, `grp_remote` (экземпляры класса `Group`).
...
| `student_sidorov` | `grp_advanced` | `comp_basic_syntax`, `comp_functions`, `comp_oop` | «отличник»: проходит весь курс, единственный с доступом к `extra_material` и `practice_3` |
...
| # | Правка | Приоритет | Связь |
|---|---|---|---|
| OPAPI1 | Обновить enum `rule_type` в `PolicyCreate` и `Policy` — добавить 5 типов | 🔴 | FIX6, FIX7, FIX8, FIX11, FIX12 |
| OPAPI2 | Добавить поля `subpolicy_ids`, `restricted_to_group_id`, `aggregate_function`, `aggregate_element_ids` в `PolicyCreate` | 🔴 | FIX6–FIX8, FIX11, FIX12 |
| OPAPI3 | Добавить схему `VerificationReport` + подсхемы (9 новых схем) | 🔴 | UC-6, B1, B2 |
| OPAPI4 | Добавить схему `BlockingExplanation` | 🟡 | UC-9, FIX9, B3 |
| OPAPI5 | Добавить схему `PolicyValidationReport` | 🟡 | UC-3, B1 |
| OPAPI6 | Добавить эндпоинты `/verify/*` | 🔴 | UC-6 |
| OPAPI7 | Добавить эндпоинты `/access/.../explain` | 🟡 | UC-9 |
| OPAPI8 | Добавить эндпоинт `POST /policies/validate` | 🟡 | UC-3 |
| OPAPI9 | Переструктурировать роутеры: выделить `/access`, `/progress` из `/integration` | 🟡 | B3, B4 |
| OPAPI10 | Расширить `ErrorResponse` на `error_code` | 🟡 | НФТ-5, FIX1, FIX9 |
| OPAPI11 | Добавить `/health`, `/health/reasoner` | 🟢 | FIX10 |
| OPAPI12 | Дополнить `OntologyMeta` полем `verifiable_properties` (СВ-1…СВ-5) | 🟢 | UC-6 UI |
| OPAPI13 | Добавить `GET /courses` и `GET /courses/{id}` | 🟢 | фронт сейчас хардкодит |

Блокирующие для фазы 2 — OPAPI1, OPAPI2, OPAPI3, OPAPI6. Остальные — параллельно.
...
| Ключ | TTL | Формула |
|---|---|---|
| `access:{s}` | адаптивный, минимум 60 с | `max(60s, min(p.valid_from, p.valid_until) - utcnow() + 5s)` где минимум берётся по `p is_active` и `rule_type=date_restricted` для студента. Если датных границ нет — TTL бесконечный (∞) до следующего E1–E10 |
| `onto:version` | ∞ | Перезаписывается `SET` без EX |
| `verify:{course_id}:{run_id}` | 3600 с | Фиксированный час — достаточно для UI-сессии методиста |
| `verify:latest:{course_id}` | 3600 с | Тот же час; указатель устаревает вместе с отчётом |
| `lock:reasoning:global` | 30 с | Защита от подвисших воркеров (падение внутри критической секции) |
| `lock:reasoning:student:{s}` | 30 с | То же |
| `recent:students` | ∞, rolling window | Чистится Z-командами, не TTL |

Бесконечный TTL для `access:{s}` при отсутствии датных границ оправдан: корректность обеспечивается событийной инвалидацией (E1–E10) + проверкой `ontology_version`. TTL здесь — защитный механизм только для датных границ, а не основной.
...
Реализация `CacheService` покрывает только базовую часть схемы (`access:{s}` + `SET`/`GET`/`DEL`). Все отсутствующие элементы зафиксированы в §А5.8 SAT_ALGORITHMS как пункты **C1–C10**; здесь — привязка к ключам схемы §8.2:

| Ключ схемы | Что реализовано в коде | Что добавить |
|---|---|---|
| `access:{s}` | SET/GET/DEL (простой JSON) | updated_at, ontology_version, duration_ms (C1); адаптивный TTL (C3); версия при GET (C4) |
| `onto:version` | — | C2 (новый ключ, хэш .owl) |
| `verify:{course_id}:{run_id}`, `verify:latest:{course_id}` | — | C9 (после реализации VerificationService, зависит от B1) |
| `lock:reasoning:global` | — | C7 (после FIX10 Docker multi-worker) |
| `lock:reasoning:student:{s}` | — | C7 |
| `recent:students` | — | C8 (опция оптимизации, не блокирует фазу 2) |

Также технический пункт C5 (замена `KEYS access:*` на `SCAN`) и C10 (startup-hook с проверкой `onto:version`). C6 — рефакторинг точки ребилда в отдельный `CacheManager` (рассогласование с C4 DSL).

Блокирующих 🔴 нет — кэш работоспособен в минимальной версии; все расхождения — улучшения надёжности и масштабируемости.
...
## 10. Что осталось по разделу 3.5.3
