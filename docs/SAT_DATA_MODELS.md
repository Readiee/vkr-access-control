...
**TBox ([1_ontology_builder.py](../code/onto/scripts/1_ontology_builder.py)):** 19 классов / 23 object properties / 14 data properties (grep 24.04).
...
**SWRL ([2_rules_setup.py](../code/onto/scripts/2_rules_setup.py)):** 14 правил через `Imp()`.

| Пункт | Статус | Примечание |
|---|---|---|
| S1 Убрать дубли `has_competency`/`targets_competency`/`is_subcompetency_of` | ✅ | Определяются только в builder |
| S2 Атомарные правила выводят `satisfies` | ✅ Блок 1 | 7 шаблонов переведены + `viewed_via_completed` (23.04) |
...
Дополнительно (решение 23.04): **H-2** `rule_competency_from_progress` — завершение элемента с `assesses=C` выдаёт студенту `has_competency=C`. Не отражено в §5.2, но добавлено в §2.2 как вспомогательное правило.
...
**Итого:** 27 пунктов из 27 закрыты. Отклонения ABox happy-path от §6.3/§6.7 (нюансы a/б/в) зафиксированы в §6.8.1.
