...
**Принцип:** количественные пороги НФТ-1..НФТ-4 заданы по UX-исследованиям восприятия задержек (Nielsen, Sirin) и не привязаны к целевому железу. Это обязательства перед пользователем, не характеристики конкретной реализации. Эмпирическое подтверждение на reference hardware фиксируется в §4.1 ПЗ и в EXP4.
...
- [x] EXP1 Верификация (СВ-1/4/5): pilot 24.04 (8 сценариев) + full 24.04 досрочно (83 сгенерированных сценария) + adversarial boundary 24.04 досрочно (19 граничных passing-кейсов). Combined 102 сценария, P=R=F1=1.0 per-property со support 10-28 positive / 10-34 negative, средняя верификация 1127 ms.
...
- [x] EXP4 Производительность: scalability UC-6 verify (10/50/100/500 правил, 3 прогона), cache hit/miss + cold_miss через AccessService+fakeredis. НФТ-1 hit 1.38 ms (цель ≤50), НФТ-2 cold miss 1397 ms (цель ≤2000), НФТ-3 verify-500 8061 ms (цель ≤10000). Все три НФТ соответствуют. 24.04 досрочно.
...
| 24.04 | EXP1 расширен до combined matrix (full 83 + adversarial 19 = 102 сценария) — отвечает на критику «regression soundness = тавтология» | После прогона full-выборки возникла проблема интерпретации: F1=1.0 на 83 генеративных сценариях означает только, что генератор и детектор согласованы по одним и тем же инвариантам (AllDisjoint, completion→access дуга, threshold вне [0, 100], subsumption по включению условий). Это regression, а не валидация относительно independent ground truth. Решение: добавить **adversarial boundary-выборку** — 19 «почти-нарушений», которые детектор обязан НЕ помечать. False positive на любом из них — настоящий баг валидатора. Группы: **A. Атомарные boundary (5):** threshold=0, threshold=100, valid_from<valid_until на 1 сек, valid_from=valid_until (точка), aggregate_required с одним элементом. **B. Near-cycle (4):** линейные цепочки completion длины 3/5/8, ромб A→B+C→D. **C. Non-redundancy (5):** одинаковые пороги на разных target'ах, разные типы (grade vs viewed) на одном target, одиночная политика, одинаковые grade на разных guarded элементах, разные группы. **D. Non-subsumption (5):** разные пороги при common group, разные target-prereq'ы, несовместимые группы, разные типы (grade vs date), разные компетенции с assesses на разных элементах. **Реализация** в `experiments/exp1_verification/adversarial.py` — не через `inject_sv*` (они валидируют параметры и кидают ValueError на валидных значениях), а через прямые билдеры onto, каждый собирает минимальный фрагмент руками. Прогон через тот же `VerificationService`. **Первый smoke выявил 2 ошибки expected, не детектора:** (а) `adv_viewed_completion_mix` (A viewed B + B completion A) — я ошибся в теории, в split-node это даёт настоящий цикл A.access → A.complete(intra) → B.access(completion) → A.access(viewed). Кейс удалён. (б) `adv_sub_distinct_comps` без `assesses` — компетенции никто не может получить, reachability=failed корректно. Добавлено `assesses=comp_X` на два разных assessor-элемента. После фикса 19/19 passed. **Результаты combined (102 сценария):** consistency TP=10/TN=34, acyclicity TP=10/TN=34, reachability TP=28/TN=34, redundancy TP=10/TN=10, subsumption TP=10/TN=10. FP=FN=0 везде. Middle-level support (10–28 positive, 10–34 negative) — достаточный для осмысленного P=R=F1=1.0, не тривиального. **Формулировка для главы 4 ПЗ:** «regression soundness на 83 кейсах + boundary robustness на 19 кейсах = комбинированная валидация detection algorithms с F1=1.0 на 102 сценариях». В перспективы переносится independent validation на реальных Moodle-курсах от Дистех (когда будет доступ к данным). **Побочный эффект**: adversarial-прогон поймал ровно те два случая, где я мог бы ошибиться в ПЗ, если бы писал анализ только по full — методологически ценно само по себе, не только как F1-усилитель. **Артефакты:** `experiments/exp1_verification/adversarial.py`, обновлённый `notebook.ipynb` с 4 новыми ячейками (adversarial generate + verify + combined matrix + обновлённый экспорт), `pz/figures/exp1/adversarial_day2.{csv}` + `combined_day2.csv` + обновлённый `full_day2.md` с тремя таблицами (full/adversarial/combined) |
| 24.04 | EXP4 Performance закрыт досрочно в тот же день: НФТ-1/2/3 все соответствуют с честной интерпретацией UC-путей | Плановая дата EXP4 — 28.04. Инструментарий — `experiments/exp4_performance/bench.py` (scalability_run + latency_run + cold_miss_run + summarize_*) плюс `notebook.ipynb` с matplotlib-графиками. Хардварный fingerprint (python 3.14.3, Windows 11, 12 CPU) сохраняется в `pz/figures/exp4/hardware.json` для ПЗ §4.1. **Замеры:** (1) Scalability UC-6 verify на курсах 10/50/100/500 политик, 3 прогона: total_mean растёт 2306 → 2764 → 3633 → 8061 ms. reasoning 1182→2310 ms (сублинейно), verify 1124→5751 ms (суперлинейно) — узкое место в post-reasoning проходах (cycle/reachability/subsumption). (2) Cache hit (30 прогонов AccessService): mean 1.38 ms, p99 1.61 ms. (3) Cache miss прогретого onto (30 прогонов): mean 4.24 ms — это AccessService._compute_inferred_access без reasoning. (4) Cold miss (5 прогонов): World + load + reasoning + AccessService с нуля: mean 1397 ms, p95 1439 ms. **Ключевая методологическая развилка:** первый прогон summary написал «НФТ-2 ПРЕВЫШЕНО» на основе UC-6 verify-500 = 8061 ms. Это ошибка интерпретации — НФТ-2 регулирует UC-4 access-read путь, не UC-6 verify. Исправление: сверка UC-6 с НФТ-3 (reasoning timeout 10 с), сверка UC-4 cold start с НФТ-2. После фикса все три НФТ в норме. **Для fakeredis**: используется как in-memory backend для AccessService замеров, а не через реальный Redis. В ПЗ §4.1 фиксируется допущение «сетевой overhead Redis localhost Unix-socket ≈ 0.1–0.3 ms из документации, пренебрежимо относительно 1.38 ms hit». Реальная Redis-интеграция протестирована в code/docker-compose.yml (Блок 3 фазы 2), но замерять через сеть в day-1 EXP4 — overkill. **Артефакты:** `experiments/exp4_performance/{bench.py,notebook.ipynb}`, `pz/figures/exp4/{summary_exp4.md, scalability_exp4.{csv,png}, latency_exp4.{csv,png}, hardware.json}`. **День 4 (плановый 27.04) освободился.** Остаются EXP2/EXP3/EXP5 в плановом окне — с учётом темпа возможно закрытие ещё в апреле |
| 24.04 | Методологическое разделение: НФТ фиксируют UX-требования (инвариантны к железу), замеры EXP4 привязаны к reference hardware из §4.1 | Уточнение после обсуждения результатов EXP4: цифры НФТ-1 ≤50 ms / НФТ-2 ≤2000 ms / НФТ-3 ≤10000 ms могут восприниматься как «выполнимо на таком-то CPU». Это неверно — они пришли из UX-работ (Nielsen «Response Times» для 50 ms, Sirin 2007 для 2 с) и не зависят от железа. Если на продакшен-железе конкретное значение не укладывается, это проблема планирования мощности или оптимизации, не постановки НФТ. **Что зафиксировано:** в §2.7.4 добавлен принцип «НФТ как UX-контракт, reference hardware в §4.1»; сохраняется формат — цифры НФТ не меняются, контекст проявляется на уровне экспериментальной части. **Что НЕ меняется:** сами значения НФТ-1..НФТ-12, их формулировки, все EXP4-сверки. `hardware.json` в `pz/figures/exp4/` остаётся эталонным артефактом reference hardware. **Следствие для защиты:** на вопрос «вы гарантируете 50 ms на Xeon?» — НФТ по Nielsen/Sirin, воспроизводится на современных CPU в пределах класса; конкретные цифры замера — §4.1; для продакшен-развёртывания у заказчика нужен повторный бенчмарк на его железе. SubsumptionChecker и reachability fixed-point имеют документированный запас оптимизации ×4–×8 без смены reasoner-а |
