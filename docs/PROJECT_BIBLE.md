...
| Блок | Пункты | Файл в коде | 🔴 | 🟡 | 🟢 |
|------|--------|-------------|----|----|-----|
| А1 Обнаружение циклов (split-node, типозависимые дуги) | G1–G10 | [graph_validator.py](../code/backend/src/services/graph_validator.py) | 5 | 3 | 2 |
| А2 Reasoning pipeline (pre-enrich, CWA, таймаут, rollback) | O1–O12 | [ontology_core.py](../code/backend/src/services/ontology_core.py), [progress_service.py](../code/backend/src/services/progress_service.py), [policy_service.py](../code/backend/src/services/policy_service.py) | 6 | 4 | 2 |
...
| А4 Достижимость (3 прохода, joint-satisfiability) | N1–N7 | — (модуль отсутствует) | 6 | 1 | 0 |
...
| А6 Redundancy/Subsumption (DL-проверка пар политик) | SUB1–SUB9 | — (модуль отсутствует) | 6 | 3 | 0 |
...
|---|---|---|---|---|---|---|
| B1 | Отсутствует [services/verification_service.py](../code/backend/src/services/) | `VerificationService` — единая точка запуска СВ-1…СВ-5 (UC-6) | СВ-1 размазан по `PolicyService`, СВ-2 — в `GraphValidator`, СВ-3/4/5 не реализованы | Добавить | 🔴 | UC-6, ФТ-3, СВ-1…СВ-5, N1, SUB1 |
| B2 | Отсутствует `api/routers/verification.py` | `VerificationController` с эндпоинтами `GET /courses/{id}/verify`, `POST /verify/subsumption` | Весь UC-6 недоступен через API | Добавить | 🔴 | UC-6, N5, SUB7 |
| B3 | Отсутствует `api/routers/access.py` | `AccessController` с `GET /access/student/{sid}/course/{cid}` (UC-4) + `GET /access/explain` (UC-9) | UC-4 сейчас через `/integration/access/...`, UC-9 (объяснение блокировки) не выделен | Добавить | 🔴 | UC-4, UC-9, ФТ-6, FIX9 |
| B4 | Отсутствует `api/routers/progress.py` | `ProgressController` с `POST /progress/events` (UC-5) | UC-5 сейчас внутри `api/routers/integration.py`, смешан с импортом структуры курса | Переделать | 🟡 | UC-5, UC-8 |
| B5 | [services/course_service.py](../code/backend/src/services/course_service.py) — гибрид | По DSL: отдельный `IntegrationService` (импорт UC-10) + чистая работа со структурой | Импорт курсов (ФТ-4, UC-10), синхронизация, чтение дерева и метаданных — в одном сервисе | Упростить | 🟡 | UC-10, ФТ-4 |
...
| B7 | [services/progress_service.py](../code/backend/src/services/progress_service.py) держит `update_progress` + кэширование + `get_student_access` | По DSL: ProgressService отвечает только за приём событий и trigger rollup; кэш — в `CacheManager`, выдача доступа — в `AccessService` | Один сервис реализует три ответственности — плохо тестируется, блокирует B1/B3 | Упростить | 🟡 | UC-4/5/8, O8, O9, C6 |
...
**Блок 2 (28–30.04, 3 дня) — верификация и новые типы правил.**

7. B1, N1–N5 — `VerificationService` + `AccessService` + `VerificationController` + эндпоинт `GET /verify`.
8. G1, G2, G4, G9, G10 — починить `GraphValidator` под типозависимую схему дуг + `find_all_cycles`.
9. SUB1–SUB4, SUB6 — модуль `SubsumptionChecker` (FIX13, FIX14).
10. FIX11 — date_restricted: O4 (снос `DateAccessFilter`), D3.
11. FIX8 — group_restricted: D2.

**Блок 3 (01–02.05, 2 дня) — композиты, фронт, тесты.**

12. FIX6/FIX7 — AND/OR + G5, G8.
13. FIX12 — aggregate_required: G6, D4.
14. F1, F2 — VerificationReport view + `api/verification.ts`.
15. F3, F5 — UC-9 в Dashboard/Simulator, новые формы в PolicyWorkspace.
16. FIX2 — тесты: U7, N7, SUB8, G10, + тесты на новые сервисы.
17. FIX15 — смоук симулятора после S2.
18. FIX10 — Docker Compose (если остаётся время).
...
| FIX13 | СВ-4 Redundancy: DL subsumption check на парах политик (алгоритм А6) | Решение 21.04 (must); элемент новизны 3 | 🔴 |
| FIX14 | СВ-5 Subsumption: subject-level поглощение (тот же А6 с другим фокусом) | Решение 21.04 (must); элемент новизны 3 | 🔴 |
...
Задачи:
- [ ] FIX1–FIX4, FIX11, FIX13, FIX14 (обязательные, 🔴): consistency, тесты, связи компетенций, циклы, date через SWRL, СВ-4/5 реализация
- [ ] FIX5–FIX9, FIX12, FIX15 (новые типы правил + производительность + смоук симулятора, 🟡)
