...
`date_restricted` — единственный тип, где кэш становится устаревшим *без события в системе*. Два подхода:
...
**Адаптивный TTL (оптимальный).** При материализации вычисляется «ближайшая датная граница», влияющая на доступ этого студента:
