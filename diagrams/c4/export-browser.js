...
(async () => {
  ...
  const url = `http://localhost:8080/workspace/1/diagrams#${firstKey}`;
  ...
  for (const key of VIEWS) {
    ...
    try {
      ...
      await page.goto(`http://localhost:8080/workspace/1/diagrams#${key}`, { waitUntil: 'networkidle2', timeout: 45000 });
      ...
      try {
        ...
        if (typeof pngDataUrl === 'string' && pngDataUrl.startsWith('data:image/png;base64,')) {
          ...
          fs.writeFileSync(path.join(OUT_DIR, 'png', `${key}.png`), Buffer.from(b64, 'base64'));
      ...
      try {
        ...
        if (svgText && typeof svgText === 'string' && svgText.trim().startsWith('<')) {
          fs.writeFileSync(path.join(OUT_DIR, 'svg', `${key}.svg`), svgText, 'utf-8');
