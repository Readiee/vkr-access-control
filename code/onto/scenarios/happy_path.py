"""Happy-path демо-курс Python.

Полный ABox: 4 студента, 3 группы, 4 компетенции с иерархией, 9 политик всех
типов, 4 модуля + 3 дополнительных элемента, 11 записей прогресса. После
запуска резонера выводятся satisfies + is_available_for.
...
def build(onto: Any) -> None:
    ...
    with onto:
        ...
        workshop.is_mandatory = False; workshop.order_index = 0
        extra = onto.Lecture("extra_material"); extra.label = ["Материал для продвинутого потока"]
        extra.is_mandatory = False; extra.order_index = 1
