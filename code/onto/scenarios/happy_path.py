...
def build(onto: Any) -> None:
    ...
    with onto:
        # --- Группы ---
        grp_standard = onto.Group("grp_standard")
        grp_advanced = onto.Group("grp_advanced")
        grp_remote = onto.Group("grp_remote")

        # --- Компетенции + иерархия ---
        comp_python = onto.Competency("comp_python")
        comp_basic_syntax = onto.Competency("comp_basic_syntax")
        comp_functions = onto.Competency("comp_functions")
        comp_oop = onto.Competency("comp_oop")
        ...
        student_petrov = onto.Student("student_petrov")
        student_sidorov = onto.Student("student_sidorov")
        student_korolev = onto.Student("student_korolev")
        ...
        # --- Структура курса ---
        course = onto.Course("course_python_basics")

        m0 = onto.Module("module_0_intro"); m0.is_required = [False]
        m1 = onto.Module("module_1_syntax"); m1.is_required = [True]
        m2 = onto.Module("module_2_functions"); m2.is_required = [True]
        m3 = onto.Module("module_3_oop"); m3.is_required = [True]
        course.has_module = [m0, m1, m2, m3]

        lec0 = onto.Lecture("lecture_0_welcome"); lec0.is_required = [True]
        m0.contains_element = [lec0]

        lec1 = onto.Lecture("lecture_1_variables"); lec1.is_required = [True]
        lec2 = onto.Lecture("lecture_2_operators"); lec2.is_required = [True]
        quiz1 = onto.Test("quiz_1"); quiz1.is_required = [True]
        quiz1.assesses = [comp_basic_syntax]
        prac1 = onto.Practice("practice_1"); prac1.is_required = [True]
        prac1.assesses = [comp_basic_syntax]
        m1.contains_element = [lec1, lec2, quiz1, prac1]

        lec3 = onto.Lecture("lecture_3_functions"); lec3.is_required = [True]
        quiz2 = onto.Test("quiz_2"); quiz2.is_required = [True]
        quiz2.assesses = [comp_functions]
        prac2 = onto.Practice("practice_2"); prac2.is_required = [True]
        prac2.assesses = [comp_functions]
        m2.contains_element = [lec3, quiz2, prac2]

        lec4 = onto.Lecture("lecture_4_classes"); lec4.is_required = [True]
        quiz3 = onto.Test("quiz_3"); quiz3.is_required = [True]
        prac3 = onto.Practice("practice_3"); prac3.is_required = [True]
        ...
        m_extras = onto.Module("module_extras"); m_extras.is_required = [False]
        course.has_module = list(course.has_module) + [m_extras]

        workshop = onto.Lecture("seasonal_workshop"); workshop.is_required = [False]
        extra = onto.Lecture("extra_material"); extra.is_required = [False]
        final_exam = onto.Test("final_exam"); final_exam.is_required = [True]
